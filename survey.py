...
def connect_spreadsheet():
    ...
    # Название таблицы на твоем Google Диске!
    return client.open("fan_id_survey_results")
...
ATTRIBUTES = {
    ...
    "Механизм бана": [
        "⚖️ Правоохранительные органы могут запретить Вам посещать матчи превентивно, даже без решения суда, просто заподозрив в намерении нарушить порядок.",
        "⚖️ Запретить Вам посещать матчи могут только на основании официально составленного протокола об административном правонарушении на стадионе.",
        "⚖️ Запретить Вам посещать матчи и аннулировать Fan ID могут исключительно по официальному решению суда."
    ],
    "Режим контроля": [
    ...
    "Цензура": [
        "🚩 Любые баннеры и элементы перформанса Вам необходимо за несколько дней до матча официально согласовывать с органами правопорядка.",
        "🚩 Предварительно согласовывать баннеры не нужно, но на входе полиция проводит жесткий досмотр и может изъять любые визуальные символы, сочтя их провокационными.",
        "🚩 Вы можете свободно проносить на трибуну баннеры, флаги и символику без предварительного согласования (если они не нарушают законы РФ)."
    ],
    "Позиция актива": [
        "🗣️ Активные фанатские группировки призывают к бойкоту: фанатские виражи пустуют, на стадионе нет привычной атмосферы организованной поддержки.",
        "🗣️ Активные фанаты присутствуют на трибунах, но в знак протеста молчат весь матч или демонстративно покидают сектора через 15 минут после начала игры.",
        "🗣️ Активные фанаты находятся на трибуне весь матч: они поют, активно используют атрибутику и гонят команду вперед, однако сохраняется высокий риск стычек."
    ],
    "Процедура билетов": [
        "🎟️ Купить билет себе или другу можно только при условии одновременного ввода номера Fan ID и сдачи биометрии. Спонтанно пойти на матч не получится.",
        "🎟️ Вы можете купить билет свободно, однако, чтобы пройти на стадион, Вам все равно придется привязать его к своему номеру Fan ID и сдать биометрию.",
        "🎟️ Вы можете купить и передать билет другу абсолютно свободно, без привязки к Fan ID и обязательной сдачи биометрических данных."
    ]
}
...
try:
    ...
    responses_ws = get_or_create_worksheet(spreadsheet, "responses", rows=5000, cols=100)
    
    # Формируем заголовки, если их еще нет
    response_headers = ['Время', 'Возраст', 'Матчей_за_5_лет', 'Клубы', 'Сектор', 'Состоит_в_фанатских_объединениях', 'Статус_FanID', 'Доход']
    for i in range(1, 6):
        for attr in ATTRIBUTES.keys(): response_headers.append(f'Pair_{i}_Match_A_{attr}')
        for attr in ATTRIBUTES.keys(): response_headers.append(f'Pair_{i}_Match_B_{attr}')
        response_headers.extend([f'Pair_{i}_Choice', f'Pair_{i}_Rating_A', f'Pair_{i}_Rating_B'])
        
    ensure_headers(responses_ws, response_headers)
except Exception as e:
    st.error("Не удалось подключиться к Google Sheets. Проверьте настройки st.secrets.")
    st.stop()
...
def save_data():
    ...
    row = [
        timestamp,
        data.get('Возраст', ''), data.get('Матчей_за_5_лет', ''), data.get('Клубы', ''),
        data.get('Сектор', ''), data.get('Состоит_в_фанатских_объединениях', ''),
        data.get('Статус_FanID', ''), data.get('Доход', '')
    ]
    
    for i in range(1, 6):
        for attr in ATTRIBUTES.keys(): row.append(data.get(f'Pair_{i}_Match_A_{attr}', ''))
        for attr in ATTRIBUTES.keys(): row.append(data.get(f'Pair_{i}_Match_B_{attr}', ''))
        row.extend([data.get(f'Pair_{i}_Choice', ''), data.get(f'Pair_{i}_Rating_A', ''), data.get(f'Pair_{i}_Rating_B', '')])
...
st.markdown(
    ...
    .vpn-banner { background: linear-gradient(135deg, #fff1ee 0%, #ffd8d1 100%); border: 1px solid rgba(191, 47, 21, 0.28); border-left: 7px solid #bf2f15; color: #7e1f10; padding: 16px 18px; border-radius: 14px; margin: 8px 0 20px 0; font-size: 1.02rem; font-weight: 600; }
    .focus-box { background: #fbf1ef; border: 1px solid rgba(191, 47, 21, 0.20); border-left: 6px solid #bf2f15; border-radius: 14px; padding: 16px 18px; margin: 8px 0 18px 0; }
    .block-title { font-size: 1.1rem; font-weight: 600; margin: 0 0 12px 0; }
    .question-gap { height: 16px; }
    .thanks-box { background: #fff3f0; border-left: 7px solid #bf2f15; border-radius: 16px; padding: 28px 24px; margin-top: 20px; }
    .thanks-title { font-size: 1.2rem; font-weight: 700; margin-bottom: 10px; color: #1f1f1f; }
    .thanks-text { color: #1f1f1f; }
    .conjoint-wrapper { display: flex; gap: 20px; align-items: stretch; margin-bottom: 20px; }
    .conjoint-card { flex: 1; border: 1px solid rgba(150, 150, 150, 0.3); border-radius: 12px; padding: 20px; background-color: transparent; }
    ...
    .conjoint-attribute { margin-bottom: 12px; font-size: 0.95rem; line-height: 1.4; }
    @media (max-width: 768px) { .conjoint-wrapper { flex-direction: column; } }
    </style>
...
if 'scroll_requested' not in st.session_state: st.session_state.scroll_requested = False
if 'conjoint_profiles' not in st.session_state:
    ...
    for _ in range(5):
        ...
        while prof_a == prof_b: prof_b = {attr: random.choice(levels) for attr, levels in ATTRIBUTES.items()}
